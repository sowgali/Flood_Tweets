...
def get_keyword_tweets(keyword, per_page=100, max_count=50000):
  ...
  while True:
    try:
      new_tweets = api.search(q=keyword, lang="en", count=per_page, max_id=max_id, result_type='recent',
                              tweet_mode='extended')
      if len(new_tweets) == 0:
        print('No new tweets')
        break
...
def analyze_tweets(tweets):
  ...
  for tweet in tweets:
    ...
    if True or (('food' in text) or ('suppl' in text) or ('medic' in text) or ('requir' in text) or ('water' in text) or ('avail' in text) or ('need' in text) or ('call' in text) or ('resource' in text) or ('contact' in text)):
      if 'sex' in text or 'porn' in text.lower() or 'callgirl' in text.lower()  or 'crore' in text.lower() or 'rupees' in text.lower() or 'million' in text.lower() or 'Eid' in text.lower() or 'vacation' in text.lower() or 'travel' in text.lower() or 'tour' in text.lower() or 'dance' in text.lower():
        continue
...
if re.match or email_match:
  if created_at.day != datetime.datetime.today().day:
    print('Skipping old tweet from: {0}'.format(created_at))
    continue
  #if match.group() == '201' and 'keralafloods2018' in text.lower()  and not email_match:
...
def collect_tweets():
  #keywords = {'kerala email':2000}
  keywords = {'KarnatakaSOS':1000, 'karnatakafloodreliefs':1000, 'karnatakafloodrelief':1000, 'karnataka floods contact':1000, 'karnataka email':1000, '#keralafloods resources':1000, 'keralarelief':1000, 'keralafloods':1000}
